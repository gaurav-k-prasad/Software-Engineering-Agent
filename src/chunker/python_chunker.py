...
class PythonChunker:
    ...
    def extract_decorator(
        ...
        for dec in node.named_children:
            if dec.type == "decorator":
                ...
                return self.extract_function(dec, decorator_names, parent_metadata)

            elif dec.type == SEMANTIC_CHUNKS["CLASS"]:
                return self.extract_class(dec, decorator_names, parent_metadata)

        return None

    def get_imports(self, node: Node) -> list[Import]:
        assert (
    ...
    def chunk_python_file(
        ...
        parent_metadata = self.init_metadata(
            root, file_path, module_path, language, dir_path
        )
        ...
        while q:
            ...
            while i < len(curr.named_children):
                ...
                if child.type == SEMANTIC_CHUNKS["FUNCTION"]:
                    chunk = self.extract_function(child, [], parent_metadata)
                    q.append((child, chunk))
                elif child.type == SEMANTIC_CHUNKS["CLASS"]:
                    chunk = self.extract_class(child, [], parent_metadata)
                    q.append((child, chunk))
                elif child.type == SEMANTIC_CHUNKS["DECORATOR"]:
                    chunk = self.extract_decorator(child, parent_metadata)
                    if chunk:
                        function_children = [
                            (item, chunk)
                    ...
                    chunk = self.init_metadata(
                        None, file_path, module_path, language, dir_path
                    )
                    ...
                    while (
                        ...
                        if child.type in ["import_from_statement", "import_statement"]:
                            chunk.imports.extend(self.get_imports(child))
                        i += 1
